class Seat: #seat class
    ...
    def set_occupant(self,name:str) -> bool:  #name is a string. If seat is unoccupant, assign it to a person(name) 
    ...
    def remove_occupant(self) ->str:
        name=self.occupant  #the person who seated down here now but will goes to other seat after
...
class Table:
    ...
    def assign_seat(self,name:str) ->bool: #name is a string. If seat is not free,
        for seat in seats: #check all seats
            if seat.free:
                seat.set.occupant(name) #using the set_occupant method of Class Seat to assign this empty seat to a name
                return True
        return False 


    def left_capacity(self):
        return sum(1 for seat in self.seats if seat.free) # plus 1 once there is a empty seat then get a sum
